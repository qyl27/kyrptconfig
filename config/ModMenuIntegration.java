...
import net.kyrptonaught.cmdkeybind.config.config.BooleanItem;
import net.kyrptonaught.cmdkeybind.config.config.ConfigScreen;
import net.kyrptonaught.cmdkeybind.config.config.KeybindItem;
...
public class ModMenuIntegration implements ModMenuApi {
    ...
    private static Screen buildScreen(Screen screen) {
        ...
        ConfigScreen configScreen = new ConfigScreen(new LiteralText("pp"));
        configScreen.addConfigItem(new BooleanItem(new TranslatableText("key.cmdkeybind.config.enabled"), options.enabled, true).setSaveConsumer(value -> options.enabled = value));
        configScreen.addConfigItem(new KeybindItem(new TranslatableText("key.cmdkeybind.config.macro.key"), options.macros.get(0).keyName, ""));
        return configScreen;
    }
